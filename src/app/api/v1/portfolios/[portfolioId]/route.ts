...
export async function GET(
  ...
  const baseQuery = supabase
    ...
    .select(
      `id, slug, title, summary, description, status, visibility, template_id, template_customization,
       ...
       portfolio_images(id, display_path, thumb_path, caption, width, height, sort_order, is_cover, created_at),
  ...
  const images = ((p.portfolio_images as unknown[]) ?? [])
    .map((img) => toCamelCaseKeys(img) as Record<string, unknown>)
    .sort((a, b) => ((a.sortOrder as number) ?? 0) - ((b.sortOrder as number) ?? 0));

  const tags = ((p.portfolio_tags as Array<{ tag_id: string; tags: unknown }>) ?? []).map(
    (pt) => toCamelCaseKeys(pt.tags) as Record<string, unknown>
  );

  const ownerProfile = toCamelCaseKeys(p.profiles) as Record<string, unknown>;
